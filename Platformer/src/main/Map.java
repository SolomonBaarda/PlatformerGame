...
public class Map {
	...
	public void render(RenderHandler renderer, GameObject[] objects, int xZoom, int yZoom) {
		...
		renderer.renderSprite(background, 0, 0, 1, 1, true, camera.getX(), camera.getY(), camera.getWidth(), camera.getHeight() );
	...
	private class Block {
		...
		public void render(RenderHandler renderer, int layer, int tileWidth, int tileHeight, int xZoom, int yZoom) {
			if(mappedTilesByLayer.length > layer) {
				...
				for(int i = 0; i < mappedTiles.size(); i++) {
					MappedTile mappedTile = mappedTiles.get(i);
					tileSet.renderTiles(mappedTile.getId(), renderer, mappedTile.getX() * tileWidth, mappedTile.getY() * tileHeight, xZoom, yZoom);
				}
			}
		}
