...
public class RenderHandler {
	 ...
	 * @param height
	 */
	public void renderSprite(Sprite sprite, int rotation, int xPos, int yPos, int xZoom, int yZoom, 
			boolean fixed, int xOffset, int yOffset, int width, int height) {

		int[] spritePixels = sprite.getPixels();
		int[] rotatedPixels = new int[spritePixels.length];
			...
			if(rotation == 1) {
				...
				for(int y=0; y<sprite.height; y++)
					for(int x=0; x<sprite.width; x++) {
						rotatedPixels[x + (y * sprite.width)] = spritePixels[1 / (x + (y * sprite.width))];
					}
			}
		}
		else {
			rotatedPixels = spritePixels;
		}


		renderArray(rotatedPixels, sprite.getWidth(), sprite.getHeight(), xPos, yPos, xZoom, yZoom, 
